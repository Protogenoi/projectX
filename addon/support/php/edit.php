...
if (isset($EXECUTE) && $EXECUTE == 1) {
    ...
    if (in_array($hello_name, $Level_10_Access, true)) {
        ...
        $assigned = filter_input(INPUT_POST, 'assigned', FILTER_SANITIZE_STRING);
    }
    $ticketStatus = filter_input(INPUT_POST, 'ticketStatus', FILTER_SANITIZE_STRING);

    $createSupportTicket = new ADL\supportTickets($pdo);
    $createSupportTicket->setAddedBy($hello_name);
    $createSupportTicket->setId($ticketID);
    $createSupportTicket->setContent($content);
    $createSupportTicket->setTicketStatus($ticketStatus);
    $createSupportTicket->setCategory($category);
    // CHECK ACCESS LEVEL TO CHANGE ASSIGNED
    if (in_array($hello_name, $Level_10_Access, true)) {
        $createSupportTicket->setAssigned($assigned);
    }
    $result = $createSupportTicket->updateTicket();

    if ($result == 'success') {

        $toastrTitle = 'Ticket updated!';
        $toastrMessage = 'Ticket message saved';
        $toastrResponse = 1;
