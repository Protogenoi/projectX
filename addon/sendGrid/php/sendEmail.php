...
if (isset($EXECUTE)) {
    ...
    if (isset($CID)) {
        ...
        $query = $pdo->prepare("SELECT client_id from client_details WHERE client_id=:CID");
        ...
        if ($EXECUTE == 1) {
            ...
            $email = new Mail();
            $email->setFrom("info5@renew-life.com ", 'Project X');
            $email->setSubject($emailSubject);
            $email->setFooter(true, 'TEXT', '<br>--<br><b>Regards,<br>' . $hello_name . '</b>');
            $email->addCategory('Project X Email');
            ...
            if (isset($_FILES['fileToUpload'])) {
                if (isset($UPLOAD_CHECK)) {
                    ...
                    for ($i = 0; $i < $countfiles; $i++) {
                        ...
                        $att1 = new Attachment();
                        $att1->setContent(file_get_contents($_FILES["fileToUpload"]["tmp_name"],
      ...
      <tr>
        ...
        try {
            ...
            if (isset($cbClient) && $cbClient == 1) {
                ...
                $clientNoteMessage = $NEW_MSG;
                $clientNoteNoteType = "Email Sent";
                $clientNoteAddedBy = $hello_name;
                $clientNoteReference = $clientName;
                ...
                $query = $pdo->prepare("INSERT into client_note set client_id=:CID, note_type='Email Sent', client_name=:ref, message=:message, sent_by=:sent");
                $query->bindParam(':CID', $CID, PDO::PARAM_STR);
                $query->bindParam(':sent', $hello_name, PDO::PARAM_STR);
                $query->bindParam(':message', $NEW_MSG, PDO::PARAM_STR);
                $query->bindParam(':ref', $clientName, PDO::PARAM_STR);
                $query->execute() or die(print_r($query->errorInfo(), true));
            ...
            if (isset($cbClient) && $cbClient == 1) {
                ...
                $clientNoteMessage = $NEW_MSG;
                $clientNoteNoteType = "Email failed";
                $clientNoteAddedBy = $hello_name;
                $clientNoteReference = $clientName;
                ...
                $query = $pdo->prepare("INSERT into client_note set client_id=:CID, note_type='Email failed', client_name=:ref, message=:message, sent_by=:sent");
                $query->bindParam(':CID', $CID, PDO::PARAM_STR);
                $query->bindParam(':sent', $hello_name, PDO::PARAM_STR);
                $query->bindParam(':message', $NEW_MSG, PDO::PARAM_STR);
                $query->bindParam(':ref', $clientName, PDO::PARAM_STR);
