...
$ADL_PAGE_TITLE = "Closer Tracker History";
    ...
    if ($EXECUTE == 1) {
        ...
        <div class="container">
            ...
            <div class="list-group">
                <span class="label label-primary"><?php echo $hello_name; ?> Trackers</span>
                <br><br>
                        ...
                        <div class="col-md-4">
                            <div class="btn-group">
                                <button type="submit" class="btn btn-success btn-sm"><i
                                        class="fa fa-calendar-alt"></i> Search Dates
                                </button>
                            </div>
                        </div>
                </form>
        ...
        <?php if (isset($TrackerEdit)) { ?>


            <div class="list-group">

            <form method="post" action="php/Tracker.php?EXECUTE=edit">
            ...
            if ($TRACKER_EDIT->rowCount() > 0) {
                ...
                <td><select name="INSURER" class="form-control" required>
                        ...
                        <option value="Aegon" <?php if (isset($TRK_EDIT_INSURER) && $TRK_EDIT_INSURER == "Aegon") {
                            echo "selected";
                        } ?> >Aegon
                        </option>
                        <option
                        ...
                        <option value="LV" <?php if (isset($TRK_EDIT_INSURER) && $TRK_EDIT_INSURER == "LV") {
                            echo "selected";
                        } ?> >LV
                        </option>
                        <option value="Aviva" <?php if (isset($TRK_EDIT_INSURER) && $TRK_EDIT_INSURER == "Aviva") {
                            echo "selected";
                        } ?> >Aviva
                        </option>
                        <option
        ...
        <?php
        $TRACKER = $pdo->prepare("SELECT insurer, mtg, lead_up, date_updated, tracker_id, agent, closer, client, phone, current_premium, our_premium, comments, sale, date_updated FROM closer_trackers WHERE closer=:closer AND DATE(date_added) BETWEEN :dateFrom AND :dateTo ORDER BY date_added");
        $TRACKER->bindParam(':closer', $hello_name, PDO::PARAM_STR);
        $TRACKER->bindParam(':dateFrom', $dateFrom, PDO::PARAM_STR);
        $TRACKER->bindParam(':dateTo', $dateTo, PDO::PARAM_STR);
        $TRACKER->execute();
