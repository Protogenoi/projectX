...
ORDER BY closer DESC");
    ...
    if ($davidXfer > 0) {
        $chart .= "{ y: 'David', a: $davidSale, b: $davidXfer},";
    }
} elseif (isset($EXECUTE) && $EXECUTE == 1 && $CLOSER == 'All') {
...
if (isset($totalXfers) && $totalXfers > 0 && isset($totalSales) && $totalSales > 0) {
...
$ADL_PAGE_TITLE = "Closer Trackers";
                    ...
                    <div class="col-md-12">
                        <div class="col-md-4">
                            <select class="form-control" name="CLOSER" id="CLOSER">
                                <option value="All" <?php if ($CLOSER == 'All') {
                                    echo "selected";
                                } ?>>All
                                </option>
                                <?php
...
WHERE
    position = 'Closer' AND employed = '1'
        AND company = :COMPANY");
                                ...
                                while ($result = $CLO_QRY->fetch(PDO::FETCH_ASSOC)) {
                                    ...
                                    if (isset($result['firstname'])) {
                                        ...
                                        if ($CLOSER_NAME == 'Michael') {
                                            $CLOSER_NAME = 'Mike';
                                        } elseif ($CLOSER_NAME == 'Daniel') {
                                            $CLOSER_NAME = 'Dan';
                                        }

                                        ?>
                                        <option value='<?php if (isset($CLOSER_NAME)) {
                                            echo $CLOSER_NAME;
                                        } ?>' <?php if ($CLOSER_NAME == $CLOSER) {
                                            echo "selected";
                                        } ?> ><?php if (isset($CLOSER_NAME)) {
                                                echo $CLOSER_NAME;
                                            } ?></option>
                                    <?php }
                                } ?>
                        ...
                        <div class="col-sm-4">
                            <input type="text" id="dateFrom" name="dateFrom" value="<?php if (isset($dateFrom)) {
                                echo "$dateFrom";
                            } else {
                                echo date("Y-m-d");
                            } ?>" class="form-control">
                ...
                <?php if (isset($dateFrom) && $hello_name != 'Keith') { ?>
                    <a
                        class="btn btn-default btn-sm"
                        href="Export/Closers.php?EXECUTE=1&dateFrom=<?php if (isset($dateFrom)) {
                            echo "$dateFrom";
                        } else {
                            echo date("Y-m-d");
                        } ?>&dateTo=<?php if (isset($dateTo)) {
                            echo "$dateTo";
                        } else {
                            echo date("Y-m-d");
                        } ?>"
                    ...
                    <a
                        class="btn btn-default btn-sm"
                        href="Export/Closers.php?EXECUTE=2&dateFrom=<?php if (isset($dateFrom)) {
                            echo "$dateFrom";
                        } else {
                            echo date("Y-m-d");
                        } ?>&dateTo=<?php if (isset($dateTo)) {
                    ...
                    ><i class="fa fa-file-excel"></i> Export Basic</a>
                    <a class="btn btn-danger btn-sm" href="?EXECUTE=1"><i class="fa fa-recycle"></i> RESET</a>
                <?php }
                if ($hello_name != 'Keith') { ?>
                    <a class="btn btn-info btn-sm" href="Agent.php?EXECUTE=1"><i class="fa fa-check-circle"></i>
                        Agent Trackers</a>
                <?php } ?>
            </div>
        </div>
        ...
        <div class="container-fluid">
            ...
            if (isset($dateFrom)) {
                if (isset($CLOSER)) {
                    ...
                    //SELECT SINGLE CLOSER BY DATE

                    if ($CLOSER != 'All') {
                        $CLO_CHK = $pdo->prepare("SELECT tracker_id from closer_trackers WHERE DATE(date_updated) BETWEEN :date AND :dateTo AND closer=:closer");
                        $CLO_CHK->bindParam(':date', $dateFrom, PDO::PARAM_STR);
                        $CLO_CHK->bindParam(':dateTo', $dateTo, PDO::PARAM_STR);
                        $CLO_CHK->bindParam(':closer', $CLOSER, PDO::PARAM_STR);
                        $CLO_CHK->execute();
                        if ($CLO_CHK->rowCount() > 0) {

                            // TRACKER TOTALS FOR NO QUOTE, HANGUP AND INSURER

                            require_once(__DIR__ . '/models/trackers/CLOSER/StatsPAD.php');
                            $StatsPad = new STATSPadModal($pdo);
                            $StatsPadList = $StatsPad->getSTATSPad($dateFrom, $dateTo, $CLOSER);
                            require_once(__DIR__ . '/views/trackers/CLOSER/Stats-PAD.php');

                            // PULL CLOSER TRACKERS

                            require_once(__DIR__ . '/models/trackers/CLOSER/CloserPAD.php');
                            $CloserPad = new CLOSERPadModal($pdo);
                            $CloserPadList = $CloserPad->getCLOSERPad($dateFrom, $dateTo, $CLOSER);
                            require_once(__DIR__ . '/views/trackers/CLOSER/Closer-PAD.php');
                        }
                    }

                    // SELECT ALL CLOSERS BY DATE

                    if ($CLOSER == 'All') {
                        $CLO_CHK = $pdo->prepare("SELECT tracker_id from closer_trackers WHERE DATE(date_updated) BETWEEN :date AND :dateTo");
                        $CLO_CHK->bindParam(':date', $dateFrom, PDO::PARAM_STR);
                        $CLO_CHK->bindParam(':dateTo', $dateTo, PDO::PARAM_STR);
                        $CLO_CHK->execute();
                        if ($CLO_CHK->rowCount() > 0) {
                            ...
                            $AllStatsPadList = $AllStatsPad->AllgetSTATSPad($dateFrom, $dateTo);
                            ...
                            $CloserPadList = $CloserPad->AllgetCLOSERPad($dateFrom, $dateTo);
                            require_once(__DIR__ . '/views/trackers/CLOSER/Closer-PAD.php');
            ...
            // EDIT A SELECT TRACKER BY TRACKER ID
            ...
            if (isset($TID) && is_numeric($TID)) {
                ...
                require_once(__DIR__ . '/views/trackers/CLOSER/editCloserTracker-view.php');

            }

            // ELSE SELECT TODAY'S CLOSER TRACKERS
            ...
            if (!isset($dateFrom)) {
                ...
                if ($CLO_CHK->rowCount() > 0) {
                    ...
                    if ($hello_name == 'xRichard') {
                        $CloserPad->setHello($hello_name);
                    }
