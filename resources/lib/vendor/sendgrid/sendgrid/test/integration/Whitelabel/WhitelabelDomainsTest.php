...
class WhitelabelDomainsTest extends BaseTestClass
{
  ...
  "default": false
        ...
        $request_headers = ["X-Mock: 200"];
        $response = self::$sg->client->whitelabel()->domains()->_($domain_id)->patch($request_body, null,
            $request_headers);
        $this->assertEquals(200, $response->statusCode());
    }
  ...
  "ip": "192.168.0.1"
        ...
        $id = "test_url_param";
        $request_headers = ["X-Mock: 200"];
        $response = self::$sg->client->whitelabel()->domains()->_($id)->ips()->post($request_body, null,
            $request_headers);
        $this->assertEquals(200, $response->statusCode());
    }
    ...
    public function testWhitelabelDomainsIdIpsDeleteMethod()
    {
        ...
        $ip = "test_url_param";
        $request_headers = ["X-Mock: 200"];
        $response = self::$sg->client->whitelabel()->domains()->_($id)->ips()->_($ip)->delete(null, null,
            $request_headers);
        $this->assertEquals(200, $response->statusCode());
    }
