...
class CampaignsTest extends BaseTestClass
{
  ...
  "send_at": 1489451436
        ...
        $campaign_id = "test_url_param";
        $request_headers = ["X-Mock: 200"];
        $response = self::$sg->client->campaigns()->_($campaign_id)->schedules()->patch($request_body, null,
            $request_headers);
        $this->assertEquals(200, $response->statusCode());
    }
  ...
  "send_at": 1489771528
        ...
        $campaign_id = "test_url_param";
        $request_headers = ["X-Mock: 201"];
        $response = self::$sg->client->campaigns()->_($campaign_id)->schedules()->post($request_body, null,
            $request_headers);
        $this->assertEquals(201, $response->statusCode());
    }
    ...
    public function testCampaignsIdSchedulesNowPostMethod()
    {
        $campaign_id = "test_url_param";
        $request_headers = ["X-Mock: 201"];
        $response = self::$sg->client->campaigns()->_($campaign_id)->schedules()->now()->post(null, null,
            $request_headers);
        $this->assertEquals(201, $response->statusCode());
    }
