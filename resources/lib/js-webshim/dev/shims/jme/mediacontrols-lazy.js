webshims.register('mediacontrols-lazy', function ($, webshims, window, doc, undefined) {
		...
		captions: {
			...
			_create: function (control, media, base) {
				...
				webshims.ready('track', function () {
					...
					function updateMode() {
						if (!menuObj || !menuObj.menu || !menuObj.menu.length) {
							return;
						}
	...
	$.jme.defineMethod('getMediaChapters', function (success) {
		...
		var updateChapterTrack = (function () {
			...
			var update = function () {
				...
				if (currentChapterTrack === selectedChapterTrack) {
					return;
				}
	...
	$.jme.defineProp('fullscreen', {
		set: function (elem, value) {
			...
			if ((!data || !data.player) && !$(elem).hasClass('player-fullscreen')) {
				return 'noDataSet';
			}
			if (value) {
				if (data.player.hasClass('player-fullscreen')) {
					return 'noDataSet';
				}
				...
				if (data.player && !data.player.hasClass('player-fullscreen')) {
					return 'noDataSet';
				}
	...
	(function () {
		var activity = {
			add: function (elem, cfg, name) {
				var data = $.data(elem, 'jmeuseractivity') || $.data(elem, 'jmeuseractivity', {
						idletime: 2500,
						idle: true,
						trigger: {}
					}),
					...
					setActive = function (e) {
						if (!e || (e.type === 'mousemove' && e.pageX === x && e.pageY === y)) {
							return;
						}
						if (e.type === 'mousemove') {
							x = e.pageX;
