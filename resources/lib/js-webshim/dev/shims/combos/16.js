...
var swfmini = function () {
	var wasRemoved = function () {
		webshims.error('This method was removed from swfmini');
	};
	var UNDEF = "undefined",
		OBJECT = "object",
...
webshims.register('dom-extend', function ($, webshims, window, document, undefined) {
	...
	[{name: 'getNativeElement', prop: 'nativeElement'}, {
		...
		prop: 'shadowElement'
	}, {name: 'getShadowFocusElement', prop: 'shadowFocusElement'}].forEach(function (data) {
				...
				if (!desc.set) {
					desc.set = desc.writeable ?
						...
						(webshims.cfg.useStrict && prop == 'prop') ?
							function () {
								throw(prop + ' is readonly on ' + nodeName);
							} :
							function () {
								webshims.info(prop + ' is readonly on ' + nodeName);
							}
					;
				}
				if (!desc.get) {
...
//contsrain-validation-api
	...
	var validityRules = {
			...
			patternMismatch: function (input, val, cache) {
				var i;
				var ret = false;
				if (val === '' || cache.nodeName == 'select') {
					return ret;
				}

				cacheType(cache, input[0]);

				if (!patternTypes[cache.type]) {
					return ret;
				}
	...
	$.each({tooShort: ['minLength', -1], tooLong: ['maxLength', 1]}, function (name, props) {
		validityRules[name] = function (input, val, cache) {
			...
			if (!val || cache.nodeName == 'select' || input.prop('defaultValue') == val) {
				return false;
			}

			cacheType(cache, input[0]);

			if (!lengthTypes[cache.type]) {
				return false;
			}
	...
	$.event.special.invalid = {
		...
		handler: function (e, d) {
			...
			if (e.type != 'submit' || e.testedValidity || !e.originalEvent || !$.nodeName(e.target, 'form') || $.prop(e.target, 'noValidate')) {
				return;
			}
	...
	$.event.special.submit = $.event.special.submit || {
		setup: function () {
			return false;
		}
	};
...
//ToDo: add object to this list
	...
	[
		...
		var changeSubmitter = function () {
			var elem = this;
			if (!('form' in elem) || !submitterTypes[elem.type]) {
				return;
			}
...
;webshims.register('form-message', function ($, webshims, window, document, undefined, options) {
	...
	var toLocale = (function () {
		...
		[{n: 'date', f: 'toLocaleDateString'}, {n: 'time', f: 'toLocaleTimeString'}, {
			n: 'datetime-local',
			f: 'toLocaleString'
		}].forEach(function (desc) {
		...
		if (window.Intl && Intl.DateTimeFormat) {
			monthFormatter = new Intl.DateTimeFormat(navigator.browserLanguage || navigator.language, {
				year: "numeric",
				month: "2-digit"
			}).format(new Date());
...
;webshims.register('form-datalist', function ($, webshims, window, document, undefined, options) {
	...
	(function () {
		...
		if (listSupport && !formsCFG.customDatalist) {
			return;
		}
					...
					list: {
						attr: {
							...
							set: function (value) {
								...
								webshims.objectCreate(options.shadowListProto, undefined, {
									input: elem,
									id: value,
									datalist: $.prop(elem, 'list')
								});
		...
		var shadowListProto = {
			_create: function (opts) {
				...
				if (noDatalistSupport[$.prop(opts.input, 'type')] || noDatalistSupport[$.attr(opts.input, 'type')]) {
					return;
				}
...
;(function (webshims) {
	...
	webshims.register('mediaelement-core', function ($, webshims, window, document, undefined, options) {
		...
		var stepSources = function (elem, data, srces) {
			var i, src;
			var testOrder = [{test: 'canNativePlaySrces', activate: 'native'}, {
		...
		var selectSource = function (elem, data) {
			...
			if (!_srces.length || !parent || parent.nodeType != 1 || stopParent.test(parent.nodeName || '')) {
				return;
			}
		...
		$(document).on('ended', function (e) {
			...
			if (supportsLoop && (!data || data.isActive == 'html5') && !$.prop(e.target, 'loop')) {
				return;
			}
			setTimeout(function () {
				if ($.prop(e.target, 'paused') || !$.prop(e.target, 'loop')) {
					return;
				}
