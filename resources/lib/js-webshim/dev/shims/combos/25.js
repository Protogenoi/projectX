...
;webshims.register('mediaelement-jaris', function ($, webshims, window, document, undefined, options) {
	...
	var onEvent = {
		...
		onDataInitialized: function (jaris, data) {
			...
			if (oldDur == data.duration || isNaN(data.duration)) {
				return;
			}

			if (data._calledMeta && ((durDelta = Math.abs(data.lastDuration - data.duration)) < 2)) {
				return;
			}
	...
	mediaelement.createSWF = function (elem, canPlaySrc, data) {
		...
		if (data && data.swfCreated) {
			...
			if (hasNative) {
				$.extend(data, {
					volume: $.prop(elem, 'volume'),
					muted: $.prop(elem, 'muted'),
					paused: $.prop(elem, 'paused')
				});
			}
	...
	['audio', 'video'].forEach(function (nodeName) {
		...
		var createGetProp = function (key) {
			if (nodeName == 'audio' && (key == 'videoHeight' || key == 'videoWidth')) {
				return;
			}
