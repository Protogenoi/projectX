    ...
    (function () {
        ...
        $.event.special.mousepress = {
            ...
            handler: function (elem, e) {
                return $.event.dispatch.call(elem, {
                    type: 'mousepress',
                    target: e.target,
                    pageX: e.pageX,
                    pageY: e.pageY
...
webshims.register('forms-picker', function ($, webshims, window, document, undefined, options) {
    ...
    picker.getYearList = function (value, data) {
        ...
        for (j = 0; j < size; j++) {
            if (j) {
                ...
                prevDisabled = picker.isInRange([start - 1], max, min) ? {
                    'data-action': 'setYearList',
                    'value': start - 1
                } : false;
            }

            str += '<div class="year-list picker-list ws-index-' + j + '"><div class="ws-picker-header"><select data-action="setYearList" class="decade-select">' + picker.createYearSelect(value, max, min, '', {
                start: start,
                step: 12 * size,
                label: start + ' – ' + (start + 11)
            }).join('') + '</select><button disabled="disabled"><span>' + start + ' – ' + (start + 11) + '</span></button></div>';
            lis = [];
            for (i = 0; i < 12; i++) {
                val = start + i;
                classArray = [];
                if (!picker.isInRange([val], max, min) || (triggerValueValidation && $(data.orig).triggerHandler('validatevalue', [{
                    value: val,
                    valueAsDate: null,
                    isPartial: [val]
                }]))) {
            ...
            if (j == size - 1) {
                nextDisabled = picker.isInRange([val + 1], max, min) ? {
                    'data-action': 'setYearList',
                    'value': val + 1
                } : false;
            }
    ...
    picker.getMonthList = function (value, data) {
        ...
        for (j = 0; j < size; j++) {
            if (j) {
                ...
                prevDisabled = picker.isInRange([value - 1], max, min) ? {
            ...
            if (j == size - 1) {
                nextDisabled = picker.isInRange([value + 1], max, min) ? {
                    'data-action': 'setMonthList',
                    'value': value + 1
                } : false;
            }
            ...
            for (i = 0; i < 12; i++) {
                ...
                classArray = [];
                if (!picker.isInRange([value, val], max, min) || (triggerValueValidation && $(data.orig).triggerHandler('validatevalue', [{
                    value: value + '-' + val,
                    valueAsDate: data.asDate(value + '-' + val),
                    isPartial: isPartial && [value, val]
                }]))) {
                    disabled = ' disabled=""';
                } else {
                    disabled = '';
    ...
    picker.getDayList = function (value, data) {
        ...
        yearNext = picker.isInRange(yearNext, max, min) ? {
            'data-action': 'setDayList',
            'value': yearNext.join('-')
        } : false;

        yearPrev = [(value[0] * 1) - 1, value[1]];
        yearPrev = picker.isInRange(yearPrev, max, min) ? {
            'data-action': 'setDayList',
            'value': yearPrev.join('-')
        } : false;
        ...
        for (j = 0; j < size; j++) {
            ...
            if (!j) {
                ...
                prevDisabled = picker.isInRange(dateArray, max, min) ? {
                    'data-action': 'setDayList',
                    'value': dateArray[0] + '-' + dateArray[1]
                } : false;
            }
            ...
            for (i = 0; i < 99; i++) {
                ...
                if (!picker.isInRange(dateArray, max, min) || (triggerValueValidation && $(data.orig).triggerHandler('validatevalue', [{
                    value: dateArray.join('-'),
                    valueAsDate: date,
                    isPartial: isPartial && dateArray
                }]))) {
                    buttonStr += ' disabled=""';
                }
            ...
            if (j == size - 1) {
                dateArray = getDateArray(date);
                dateArray[2] = 1;
                nextDisabled = picker.isInRange(dateArray, max, min) ? {
                    'data-action': 'setDayList',
                    'value': dateArray.date
                } : false;
            }
        }
...
//	var createDatimeValue =
    ...
    picker.getTimeList = function (value, data) {
        ...
        for (; i <= len; i++) {
            ...
            if (!data.isValid(hVal, attrs) || (triggerValueValidation && $(data.orig).triggerHandler('validatevalue', [{
                value: hVal,
                valueAsDate: data.asDate(hVal),
                partial: false
            }]))) {
                str += ' disabled=""';
            }
                ...
                $('button', popover.buttonRow).each(function () {
                    ...
                    if ($(this).hasClass('ws-empty')) {
                        ...
                        if (today[data.type] && data.type != 'time') {
                            $.prop(this, 'disabled', (!picker.isInRange(today[data.type].split('-'), o.maxS, o.minS) || !!$(data.orig).triggerHandler('validatevalue', [{
                                value: today[data.type],
